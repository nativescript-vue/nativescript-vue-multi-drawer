    ...
    computed: {
        computedSidesEnabled() {
            ...
            return Object.keys(this.$slots).filter(
                (slotName) => validSides.includes(slotName) && this.optionsInternal[slotName].enabled
            );
        },
        computedDrawerStyle() {
            return (side) => ({
                transform: `translate${this.optionsInternal[side].axis}(${
                    this.sides[side].open || this.optionsInternal[side].fixed ? 0 : this.sides[side].translationOffset
                })`,
        ...
        computedSwipeOpenTriggerProperties() {
            return (side) => ({
                ...
                ...(this.optionsInternal[side].swipeOpenTriggerHeight
                    ...
                    : {}),
                [this.optionsInternal[side].axis === 'X' ? 'horizontalAlignment' : 'verticalAlignment']: side,
                ...(this.optionsInternal.debug ? { backgroundColor: 'rgba(0, 0, 255, 0.3)' } : {}),
        ...
        computedLayout() {
          ...
          let gridColumns = (options.left.fixed ? 'auto,' : '') 
            + '*'
        ...
        onDrawerLayoutChange(side) {
            ...
            this.sides[side].translationOffset =
                ...
                utils.layout.toDeviceIndependentPixels(
                    this.optionsInternal[side].axis === 'X' ? view.getMeasuredWidth() : view.getMeasuredHeight()
                );
        },
        onBackDropPan(args) {
